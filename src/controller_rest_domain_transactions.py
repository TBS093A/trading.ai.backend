    ...
    return {
        ...
        "available_endpoints": {
            "crud": {
                ...
                "update": "PUT /transactions/{id} - Aktualizacja transakcji",
                "delete": "DELETE /transactions/{id} - Usunięcie transakcji"
            },
            "filtering": {
...
async def update_transaction(
    transaction_id: int = Path(..., ge=1, description="ID transakcji do aktualizacji"),
    transaction_data: TransactionUpdate = Body(..., description="Dane do aktualizacji")
):
    """
    Aktualizuje istniejącą transakcję.
    
    Args:
        transaction_id: ID transakcji do aktualizacji
        transaction_data: Nowe dane transakcji
    ...
    try:
        ...
        existing = await transactions_table.get_by_id(transaction_id)
        if not existing:
            raise HTTPException(
                status_code=404,
                detail=f"Transaction with ID {transaction_id} not found"
        ...
        update_data = {}
        if transaction_data.exchange_id is not None:
            update_data['exchange_id'] = transaction_data.exchange_id
        if transaction_data.asset_id is not None:
            update_data['asset_id'] = transaction_data.asset_id
        if transaction_data.exchange_account_state_id is not None:
            update_data['exchange_account_state_id'] = transaction_data.exchange_account_state_id
        if transaction_data.general_interpretation_id is not None:
            update_data['general_interpretation_id'] = transaction_data.general_interpretation_id
        if transaction_data.buy_strategy_id is not None:
            update_data['buy_strategy_id'] = transaction_data.buy_strategy_id
        if transaction_data.sell_strategy_id is not None:
            update_data['sell_strategy_id'] = transaction_data.sell_strategy_id
        if transaction_data.type is not None:
            update_data['type'] = transaction_data.type
        if transaction_data.quote_amount is not None:
            update_data['quote_amount'] = float(transaction_data.quote_amount)
        if transaction_data.asset_amount is not None:
            update_data['asset_amount'] = float(transaction_data.asset_amount)
        
        if not update_data:
            raise HTTPException(status_code=400, detail="No data provided for update")
        
        success = await transactions_table.update(transaction_id, **update_data)
...
async def delete_transaction(
    transaction_id: int = Path(..., ge=1, description="ID transakcji do usunięcia")
):
    """
    Usuwa transakcję z bazy danych.
    
    Args:
        transaction_id: ID transakcji do usunięcia
    ...
    try:
        ...
        success = await transactions_table.delete(transaction_id)
        ...
        return StandardResponse(
            success=True,
            message=f"Transaction (ID: {transaction_id}) deleted successfully",
            data={"transaction_id": transaction_id}
        )
