    ...
    return {
        ...
        "available_endpoints": {
            "crud": {
                ...
                "create": "POST /llm/interpretation/fundamental - Tworzenie nowej interpretacji",
                "update": "PUT /llm/interpretation/fundamental/{id} - Aktualizacja interpretacji",
                "delete": "DELETE /llm/interpretation/fundamental/{id} - Usunięcie interpretacji"
            },
            "filtering": {
...
async def create_fundamental_interpretation(
    interpretation_data: FundamentalAnalysisInterpretationCreate = Body(..., description="Dane nowej interpretacji")
):
    """
    Tworzy nową interpretację LLM analizy fundamentalnej.
    ...
    try:
        ...
        if not interpretation_id:
            raise HTTPException(status_code=500, detail="Failed to create fundamental interpretation")
        
        return StandardResponse(
            success=True,
            message="Fundamental interpretation created successfully",
        ...
        logger.error(f"Error creating fundamental interpretation: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to create fundamental interpretation: {str(e)}")


@router.put("/{interpretation_id}", response_model=StandardResponse)
async def update_fundamental_interpretation(
    interpretation_id: int = Path(..., ge=1, description="ID interpretacji do aktualizacji"),
    interpretation_data: FundamentalAnalysisInterpretationUpdate = Body(..., description="Dane do aktualizacji")
):
    """
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=500, detail="Failed to update fundamental interpretation")
        
        return StandardResponse(
            success=True,
            message=f"Fundamental interpretation {interpretation_id} updated successfully",
            data={"interpretation_id": interpretation_id}
        )
        ...
        logger.error(f"Error updating fundamental interpretation {interpretation_id}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to update fundamental interpretation: {str(e)}")


@router.delete("/{interpretation_id}", response_model=StandardResponse)
async def delete_fundamental_interpretation(
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=500, detail="Failed to delete fundamental interpretation")
        
        return StandardResponse(
            success=True,
            message=f"Fundamental interpretation (ID: {interpretation_id}) deleted successfully",
            data={"interpretation_id": interpretation_id}
        )
        ...
        logger.error(f"Error deleting fundamental interpretation {interpretation_id}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to delete fundamental interpretation: {str(e)}")
