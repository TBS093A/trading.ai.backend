...
class ExchangeAccountStateSellStrategiesTable(AbstractTable):
    """Klasa do zarządzania tabelą ExchangeAccountStateSellStrategies."""
    
    def create_table(self) -> str:
        return """
        CREATE TABLE IF NOT EXISTS exchange_account_state_sell_strategies (
    ...
    async def create(self, exchange_account_state_id: int, is_percent: bool, type: str, 
        ...
        """
        Tworzy nową strategię sprzedaży i zwraca jej ID.
        ...
        try:
            ...
            strategy_id = await self.fetch_val("""
                INSERT INTO exchange_account_state_sell_strategies 
                (exchange_account_state_id, is_percent, type, movement_amount) 
                VALUES ($1, $2, $3, $4) RETURNING id
            """, exchange_account_state_id, is_percent, type, movement_amount)
            
            logger.info(f"Utworzono strategię sprzedaży: {type} {movement_amount} dla account_state {exchange_account_state_id} z ID: {strategy_id}")
            return strategy_id
        except Exception as e:
            logger.error(f"Błąd podczas tworzenia strategii sprzedaży: {e}", exc_info=True)
            return None
    
    async def create_many(self, strategies: List[Dict[str, Any]]) -> Dict[str, int]:
        """
        Tworzy wiele strategii sprzedaży jednym zapytaniem.
        ...
        try:
            ...
            logger.info(f"Utworzono {len(created_strategies)} nowych strategii sprzedaży z {len(strategies)} prób")
            return created_strategies
            
        except Exception as e:
            logger.error(f"Błąd podczas tworzenia wielu strategii sprzedaży: {e}", exc_info=True)
            return {}
    
    async def get_by_id(self, record_id: int) -> Optional[Dict[str, Any]]:
        """Pobiera strategię sprzedaży po ID wraz z informacjami o stanie konta."""
        return await self.fetch_one("""
            SELECT eass.id, eass.exchange_account_state_id, eass.is_percent, eass.type, 
                   eass.movement_amount, eass.created_at, eass.updated_at,
            ...
            FROM exchange_account_state_sell_strategies eass
            JOIN exchange_account_state eas ON eass.exchange_account_state_id = eas.id
    ...
    async def get_by_exchange_account_state_id(self, exchange_account_state_id: int) -> Optional[Dict[str, Any]]:
        """Pobiera strategię sprzedaży dla konkretnego stanu konta (relacja 1:1)."""
        return await self.fetch_one("""
            SELECT eass.id, eass.exchange_account_state_id, eass.is_percent, eass.type, 
                   eass.movement_amount, eass.created_at, eass.updated_at,
                   eas.exchange_id, eas.type as account_type, eas.currency, eas.amount as account_amount,
    ...
    async def get_by_strategy_type(self, strategy_type: str, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        """Pobiera strategie sprzedaży według typu (MARKET/LIMIT)."""
    ...
    async def get_percent_based_strategies(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        return await self.fetch_all("""
            ...
            JOIN exchange_account_state eas ON eass.exchange_account_state_id = eas.id
            JOIN exchanges e ON eas.exchange_id = e.id
            WHERE eass.is_percent = TRUE
            ORDER BY eass.movement_amount DESC
            LIMIT $1 OFFSET $2
        """, limit, offset)
    ...
    async def get_absolute_amount_strategies(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        return await self.fetch_all("""
            ...
            JOIN exchange_account_state eas ON eass.exchange_account_state_id = eas.id
            JOIN exchanges e ON eas.exchange_id = e.id
            WHERE eass.is_percent = FALSE
            ORDER BY eass.movement_amount DESC
            LIMIT $1 OFFSET $2
        """, limit, offset)
    
    async def update(self, record_id: int, **kwargs) -> bool:
        """Aktualizuje strategię sprzedaży o podanym ID."""
        try:
            ...
            if not update_fields:
                logger.warning(f"Brak dozwolonych pól do aktualizacji dla strategii sprzedaży {record_id}")
                return False
            ...
            query = f"""
                UPDATE exchange_account_state_sell_strategies 
                SET {', '.join(update_fields)}
                WHERE id = ${param_counter}
            """
            
            await self.execute_query(query, *params)
            logger.info(f"Zaktualizowano strategię sprzedaży z ID: {record_id}")
            return True
        except Exception as e:
            logger.error(f"Błąd podczas aktualizacji strategii sprzedaży: {e}", exc_info=True)
            return False
    
    async def delete(self, record_id: int) -> bool:
        """Usuwa strategię sprzedaży o podanym ID."""
        try:
            await self.execute_query("DELETE FROM exchange_account_state_sell_strategies WHERE id = $1", record_id)
            logger.info(f"Usunięto strategię sprzedaży z ID: {record_id}")
            return True
        except Exception as e:
            logger.error(f"Błąd podczas usuwania strategii sprzedaży: {e}", exc_info=True)
            return False
    
    async def delete_by_exchange_account_state_id(self, exchange_account_state_id: int) -> bool:
        """Usuwa strategię sprzedaży dla konkretnego stanu konta."""
        try:
            await self.execute_query(
                "DELETE FROM exchange_account_state_sell_strategies WHERE exchange_account_state_id = $1",
                exchange_account_state_id
            )
            logger.info(f"Usunięto strategię sprzedaży dla account_state_id: {exchange_account_state_id}")
            return True
        except Exception as e:
            logger.error(f"Błąd podczas usuwania strategii sprzedaży dla account_state_id: {e}", exc_info=True)
            return False
    
    async def get_all(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        """Pobiera wszystkie strategie sprzedaży z limitem i offsetem."""
        return await self.fetch_all("""
            SELECT eass.id, eass.exchange_account_state_id, eass.is_percent, eass.type, 
                   eass.movement_amount, eass.created_at, eass.updated_at,
                   eas.exchange_id, eas.type as account_type, eas.currency, eas.amount as account_amount,
    ...
    async def calculate_sell_amount(self, strategy_id: int) -> Optional[float]:
        """
        Oblicza rzeczywistą kwotę do sprzedaży na podstawie strategii.
        
        Returns:
            float: Kwota do sprzedaży lub None jeśli strategia nie istnieje
        """
        try:
            result = await self.fetch_one("""
                SELECT eass.is_percent, eass.movement_amount,
            ...
            if result['is_percent']:
                ...
                sell_amount = (result['account_amount'] * result['movement_amount']) / 100
            else:
                # Użyj kwoty bezwzględnej
                sell_amount = result['movement_amount']
            
            logger.info(f"Obliczono kwotę sprzedaży dla strategii {strategy_id}: {sell_amount}")
            return float(sell_amount)
            
        except Exception as e:
            logger.error(f"Błąd podczas obliczania kwoty sprzedaży: {e}", exc_info=True)
            return None
