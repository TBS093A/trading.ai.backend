    ...
    return {
        ...
        "available_endpoints": {
            "crud": {
                ...
                "create": "POST /analysis/technical - Tworzenie nowej analizy",
                "update": "PUT /analysis/technical/{id} - Aktualizacja analizy",
                "delete": "DELETE /analysis/technical/{id} - Usunięcie analizy"
            },
            "filtering": {
...
async def create_technical_analysis(
    analysis_data: TechnicalAnalysisCreate = Body(..., description="Dane nowej analizy")
):
    """
    Tworzy nową analizę techniczną.
    
    Args:
        analysis_data: Dane analizy do utworzenia
        
    Returns:
        StandardResponse: Odpowiedź z ID utworzonej analizy
    """
    try:
        db = await get_db()
        ta_table = db.get_factory().get_technical_analysis_harmonic_patterns_table()
        ...
        # Utwórz analizę
        analysis_id = await ta_table.create(
            asset_id=analysis_data.asset_id,
            ta_object_json=analysis_data.ta_object_json,
            interval=analysis_data.interval,
            x_point_timestamp=analysis_data.x_point_timestamp,
            a_point_timestamp=analysis_data.a_point_timestamp,
            b_point_timestamp=analysis_data.b_point_timestamp,
            c_point_timestamp=analysis_data.c_point_timestamp,
            d_point_timestamp=analysis_data.d_point_timestamp
        )
        
        if not analysis_id:
            raise HTTPException(status_code=500, detail="Failed to create technical analysis")
        
        return StandardResponse(
            success=True,
            message="Technical analysis created successfully",
        ...
        logger.error(f"Error creating technical analysis: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to create technical analysis: {str(e)}")


@router.put("/{analysis_id}", response_model=StandardResponse)
async def update_technical_analysis(
    analysis_id: int = Path(..., ge=1, description="ID analizy do aktualizacji"),
    analysis_data: TechnicalAnalysisUpdate = Body(..., description="Dane do aktualizacji")
):
    """
    Aktualizuje istniejącą analizę techniczną.
    
    Args:
        analysis_id: ID analizy do aktualizacji
        analysis_data: Nowe dane analizy
    ...
    try:
        ...
        existing = await ta_table.get_by_id(analysis_id)
        if not existing:
            raise HTTPException(
                status_code=404,
                detail=f"Technical analysis with ID {analysis_id} not found"
        ...
        update_data = {}
        if analysis_data.asset_id is not None:
            update_data['asset_id'] = analysis_data.asset_id
        if analysis_data.ta_object_json is not None:
            update_data['ta_object_json'] = analysis_data.ta_object_json
        if analysis_data.interval is not None:
            update_data['interval'] = analysis_data.interval
        if analysis_data.x_point_timestamp is not None:
            update_data['x_point_timestamp'] = analysis_data.x_point_timestamp
        if analysis_data.a_point_timestamp is not None:
            update_data['a_point_timestamp'] = analysis_data.a_point_timestamp
        if analysis_data.b_point_timestamp is not None:
            update_data['b_point_timestamp'] = analysis_data.b_point_timestamp
        if analysis_data.c_point_timestamp is not None:
            update_data['c_point_timestamp'] = analysis_data.c_point_timestamp
        if analysis_data.d_point_timestamp is not None:
            update_data['d_point_timestamp'] = analysis_data.d_point_timestamp
        
        if not update_data:
            raise HTTPException(status_code=400, detail="No data provided for update")
        
        # Wykonaj aktualizację
        success = await ta_table.update(analysis_id, **update_data)
        
        if not success:
            raise HTTPException(status_code=500, detail="Failed to update technical analysis")
        
        return StandardResponse(
            success=True,
            message=f"Technical analysis {analysis_id} updated successfully",
            data={"analysis_id": analysis_id}
        )
        ...
        logger.error(f"Error updating technical analysis {analysis_id}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to update technical analysis: {str(e)}")


@router.delete("/{analysis_id}", response_model=StandardResponse)
async def delete_technical_analysis(
    analysis_id: int = Path(..., ge=1, description="ID analizy do usunięcia")
):
    """
    Usuwa analizę techniczną z bazy danych.
    ...
    try:
        ...
        # Usuń analizę
        success = await ta_table.delete(analysis_id)
        
        if not success:
            raise HTTPException(status_code=500, detail="Failed to delete technical analysis")
        
        return StandardResponse(
            success=True,
            message=f"Technical analysis (ID: {analysis_id}) deleted successfully",
            data={"analysis_id": analysis_id}
        )
        ...
        logger.error(f"Error deleting technical analysis {analysis_id}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to delete technical analysis: {str(e)}")
