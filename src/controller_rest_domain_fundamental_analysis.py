    ...
    return {
        ...
        "available_endpoints": {
            "crud": {
                ...
                "create": "POST /analysis/fundamental - Tworzenie nowej analizy",
                "update": "PUT /analysis/fundamental/{id} - Aktualizacja analizy",
                "delete": "DELETE /analysis/fundamental/{id} - Usunięcie analizy"
            },
            "filtering": {
...
async def create_fundamental_analysis(
    analysis_data: FundamentalAnalysisCreate = Body(..., description="Dane nowej analizy")
):
    """
    Tworzy nową analizę fundamentalną.
    ...
    try:
        ...
        # Utwórz analizę
        analysis_id = await fa_table.create(
            asset_ids=analysis_data.asset_ids,
            timestamp=analysis_data.timestamp,
            content=analysis_data.content,
            link=analysis_data.link,
            service=analysis_data.service
        )
        
        if not analysis_id:
            raise HTTPException(status_code=500, detail="Failed to create fundamental analysis")
        
        return StandardResponse(
            success=True,
            message="Fundamental analysis created successfully",
            data={"analysis_id": analysis_id, "asset_count": len(analysis_data.asset_ids)}
        )
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error creating fundamental analysis: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to create fundamental analysis: {str(e)}")


@router.put("/{analysis_id}", response_model=StandardResponse)
async def update_fundamental_analysis(
    analysis_id: int = Path(..., ge=1, description="ID analizy do aktualizacji"),
    analysis_data: FundamentalAnalysisUpdate = Body(..., description="Dane do aktualizacji")
):
    """
    Aktualizuje istniejącą analizę fundamentalną.
    ...
    try:
        ...
        update_data = {}
        if analysis_data.timestamp is not None:
            update_data['timestamp'] = analysis_data.timestamp
        if analysis_data.content is not None:
            update_data['content'] = analysis_data.content
        if analysis_data.link is not None:
            update_data['link'] = analysis_data.link
        if analysis_data.service is not None:
            update_data['service'] = analysis_data.service
        
        if not update_data:
            raise HTTPException(status_code=400, detail="No data provided for update")
        
        # Wykonaj aktualizację
        success = await fa_table.update(analysis_id, **update_data)
        
        if not success:
            raise HTTPException(status_code=500, detail="Failed to update fundamental analysis")
        
        return StandardResponse(
            success=True,
            message=f"Fundamental analysis {analysis_id} updated successfully",
        ...
        logger.error(f"Error updating fundamental analysis {analysis_id}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to update fundamental analysis: {str(e)}")


@router.delete("/{analysis_id}", response_model=StandardResponse)
async def delete_fundamental_analysis(
    analysis_id: int = Path(..., ge=1, description="ID analizy do usunięcia")
):
    """
    Usuwa analizę fundamentalną z bazy danych.
    ...
    try:
        ...
        # Usuń analizę (CASCADE usunie też powiązania z assetami)
        success = await fa_table.delete(analysis_id)
        
        if not success:
            raise HTTPException(status_code=500, detail="Failed to delete fundamental analysis")
        
        return StandardResponse(
            success=True,
            message=f"Fundamental analysis (ID: {analysis_id}) deleted successfully",
            data={"analysis_id": analysis_id}
        )
        ...
        logger.error(f"Error deleting fundamental analysis {analysis_id}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to delete fundamental analysis: {str(e)}")
